...
try:
    ...
    epanet = None
    print("[WARNING] 'epyt' (EPANET) not available. Hydraulic functions will be disabled in test environment.")
import numpy as np
...
class RemoteValveControlEnv:
    ...
    def _build_sensor_entry(self, loop_info, loop_config, measurements, step_count, current_time):
        ...
        a nested target object containing both target_pressure and target_flow.

        This centralizes payload construction so unit tests can verify it.
        """
        ...
        for i in range(max_retries):
            try:
                payload = {
                    ...
                    "simulation": {
                        "duration": self.sim_config.get('duration', 0),
                        "hydraulic_step": self.sim_config.get('hydraulic_step', 0),
                        "max_steps": int(
                            self.sim_config.get('duration', 0) /
                            max(self.sim_config.get('hydraulic_step', 1), 1)
                        ) + 1
                    }
                }
    ...
    def run(self):
        ...
        while current_time <= duration:
            ...
            for i, loop_info in enumerate(loop_data):
                ...
                # Build sensor payload (includes both pressure and flow and nested target)
                sensor_entry = self._build_sensor_entry(
                    loop_info,
                    loop_config,
                    measurements_dict,
                    step_count,
                ...
                target_value = (
                    sensor_entry['target'].get('target_flow', 100.0)
                    if self.control_mode == 'flow'
                    else sensor_entry['target'].get('target_pressure', 30.0)
                )
                controlled_value = sensor_entry['controlled_value']

                sensor_data.append(sensor_entry)

                # Keep a separate measurements list for internal bookkeeping
