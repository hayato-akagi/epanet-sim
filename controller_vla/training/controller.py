...
class VLAController:
    ...
    def __init__(self, loop_id, vla_model, agent, reward_calculator, 
        ...
        default_checkpoint_dir = os.path.join(exp_result_dir, 'checkpoints')
        configured_checkpoint_dir = checkpointing_config.get('save_dir', default_checkpoint_dir)
        # Backward compatibility: legacy configs used shared global directory.
        # Normalize to per-experiment directory for multi-episode resume behavior.
        if configured_checkpoint_dir in ('/shared/checkpoints', '/shared/results/checkpoints'):
            self.checkpoint_dir = default_checkpoint_dir
        elif os.path.isabs(configured_checkpoint_dir):
            self.checkpoint_dir = configured_checkpoint_dir
        else:
            self.checkpoint_dir = os.path.join(exp_result_dir, configured_checkpoint_dir)
        self.auto_checkpoint_enabled = checkpointing_config.get('enabled', True)
    ...
    def set_max_steps_per_episode(self, max_steps):
        """Set max steps per episode from simulation config"""
        ...
        try:
            ...
            if max_steps > 0:
                ...
                print(f"[VLAController] max_steps_per_episode updated to {self.max_steps_per_episode}")
        except Exception as e:
            print(f"[WARNING] Invalid max_steps_per_episode '{max_steps}': {e}")
    ...
    def compute_action(self, sensor_data, step, time_step, exp_id=None):
        ...
        # Determine whether we're using pressure or flow as the controlled variable.
        use_flow = False
        target_raw = sensor_data.get('target')
        if isinstance(target_raw, dict) and target_raw.get('target_flow') is not None:
            use_flow = True

        # Map the observed value into the existing 'pressure'/'target' keys so
        # downstream logic (reward, logging, buffering) remains unchanged.
        if use_flow:
            observed_value = sensor_data.get('flow', sensor_data.get('pressure'))
            target_value = target_raw.get('target_flow') if isinstance(target_raw, dict) else target_raw
            if self.prev_state is not None:
                prev_observed = self.prev_state.get('flow', self.prev_state.get('pressure'))
            else:
                prev_observed = None
            print(f"[DEBUG] Using FLOW mode for control (observed={observed_value}, target={target_value})")
        else:
            observed_value = sensor_data.get('pressure')
            # target may be nested or scalar
            if isinstance(target_raw, dict):
                target_value = target_raw.get('target_pressure')
            else:
                target_value = target_raw
            if self.prev_state is not None:
                prev_observed = self.prev_state.get('pressure')
            else:
                prev_observed = None

        # Construct state (keep keys 'pressure' and 'target' for backward compatibility)
        current_state = {
            'images': images,
            'prompt': prompt,
            'pressure': observed_value,
            'target': target_value,
            'prev_action': sensor_data.get('prev_action', 0.0),
            'control_mode': 'flow' if use_flow else 'pressure',
            'observed_value': observed_value,
            'target_value': target_value,
            'pressure_raw': sensor_data.get('pressure'),
            'target_pressure_raw': target_raw.get('target_pressure') if isinstance(target_raw, dict) else (target_value if not use_flow else None),
            # also include explicit flow keys when available
            'flow': sensor_data.get('flow'),
            'target_flow': target_raw.get('target_flow') if isinstance(target_raw, dict) else (target_value if use_flow else None)
        }
        ...
        if self.prev_state is not None:
            ...
            # Calculate reward. If using flow, pass flow values into the
            # existing pressure-based reward API (mapping is intentional to
            # maintain backward compatibility with RewardCalculator).
            if use_flow:
                reward_components = self.reward_calculator.calculate(
                    current_pressure=observed_value,
                    target_pressure=target_value,
                    prev_pressure=prev_observed,
    ...
    def step(self, state, action, reward, next_state, done, 
        ...
        step_data = {
            ...
            'time_step': time_step,
            'control_mode': state.get('control_mode', 'pressure'),
            'observed_value': state.get('observed_value', state['pressure']),
            'target_value': state.get('target_value', state['target']),
            'pressure': state['pressure'],
            'target_pressure': state['target'],
            'flow': state.get('flow'),
            'target_flow': state.get('target_flow'),
            'pressure_raw': state.get('pressure_raw', state.get('pressure')),
            'target_pressure_raw': state.get('target_pressure_raw', state.get('target')),
        ...
        # Method 1: Check done flag (if provided by sim_runner)
    ...
    def _finish_episode(self):
        ...
        if self.auto_checkpoint_enabled:
            ...
            episode_checkpoint = os.path.join(
                self.checkpoint_dir,
                f"{self.exp_id}_{self.loop_id}_ep{self.current_episode}.pt"
            )
            self.save_checkpoint(latest_checkpoint, mark_episode_end=True)
            self.save_checkpoint(episode_checkpoint, mark_episode_end=True)
    ...
    def _calculate_episode_stats(self):
        ...
        pressures = [s.get('observed_value', s['pressure']) for s in self.episode_buffer]
        targets = [s.get('target_value', s['target_pressure']) for s in self.episode_buffer]
    ...
    def get_latest_checkpoint_path(self):
        """Return path to latest checkpoint for this loop"""
        return os.path.join(self.checkpoint_dir, f"{self.exp_id}_{self.loop_id}_latest.pt")

    def _extract_vla_model_state(self):
        """Extract state dict from VLA model wrapper if available"""
        if hasattr(self.vla_model, 'state_dict'):
            return self.vla_model.state_dict()
        wrapped_model = getattr(self.vla_model, 'model', None)
        if wrapped_model is not None and hasattr(wrapped_model, 'state_dict'):
            return wrapped_model.state_dict()
        return None

    def _load_vla_model_state(self, model_state):
        """Load state dict into VLA model wrapper if possible"""
        if model_state is None:
            return
        if hasattr(self.vla_model, 'load_state_dict'):
            self.vla_model.load_state_dict(model_state)
            return
        wrapped_model = getattr(self.vla_model, 'model', None)
        if wrapped_model is not None and hasattr(wrapped_model, 'load_state_dict'):
            wrapped_model.load_state_dict(model_state)

    def save_checkpoint(self, filepath, mark_episode_end=False):
        """Save model checkpoint"""
        agent_state = self.agent.get_state() if hasattr(self.agent, 'get_state') else None
        checkpoint = {
            'vla_model_state': self._extract_vla_model_state(),
            'agent_state': agent_state,
            'episode': self.current_episode,
            'resume_episode': self.current_episode + 1 if mark_episode_end else self.current_episode,
    ...
    def load_checkpoint(self, filepath):
        ...
        self._load_vla_model_state(checkpoint.get('vla_model_state'))
        agent_state = checkpoint.get('agent_state')
        if agent_state is not None and hasattr(self.agent, 'load_state'):
            self.agent.load_state(agent_state)
        self.current_episode = checkpoint.get('resume_episode', checkpoint.get('episode', 0))
        self.step_in_episode = checkpoint.get('step_in_episode', 0)
