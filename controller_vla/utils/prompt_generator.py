class PromptGenerator:
    ...
    def _generate_pressure_prompt(self, sensor_data):
        ...
        target = self._extract_target_value(sensor_data.get('target', 30.0), mode='pressure')
...
Task: Adjust the valve to maintain target pressure."""
    ...
    def _generate_flow_prompt(self, sensor_data):
        ...
        target = self._extract_target_value(sensor_data.get('target', 100.0), mode='flow')
        valve = sensor_data.get('prev_action', 0.5)
        pressure = sensor_data.get('pressure', 30.0)
...
Task: Adjust the valve to maintain target flow."""
    ...
    def _extract_target_value(self, target, mode):
        """Extract scalar target value from numeric or nested target dict."""
        if isinstance(target, (int, float)):
            return float(target)

        if isinstance(target, dict):
            if mode == 'flow':
                value = target.get('target_flow', target.get('flow', 100.0))
            else:
                value = target.get('target_pressure', target.get('pressure', 30.0))
            if isinstance(value, (int, float)):
                return float(value)

        return 100.0 if mode == 'flow' else 30.0