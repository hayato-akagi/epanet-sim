...
def render_training_progress(steps_df, episodes_df):
    ...
    if steps_df is None and episodes_df is None:
        ...
        **Expected files:**
        ...
        These files are automatically generated by controller_vla during training.
        """)
        return
    ...
    if steps_df is not None and episodes_df is not None:
        tab1, tab2, tab3 = st.tabs(["📈 Learning Curves", "📊 Episode Analysis", "🔍 Detailed Metrics"])
...
def render_training_summary(steps_df, episodes_df):
    ...
    if episodes_df is not None and len(episodes_df) > 0:
        ...
        st.info("📊 Episode statistics will appear here after completing at least one episode.")
...
def render_learning_curves(steps_df):
    ...
    if steps_df is None or len(steps_df) == 0:
        st.info("📈 Learning curves will appear here as training progresses.")
        st.markdown("""
    ...
    if len(valid_actor) > 0 or len(valid_critic) > 0:
        ...
        if len(valid_actor) > 0:
            ...
            st.info(f"💡 Learning started at step {first_learning_step:,}")
    else:
        ...
        if 'delta_action' in steps_df.columns:
            ...
            fig_action.add_trace(
                go.Scatter(x=steps_df['total_steps'], y=ma_action,
                          mode='lines', name=f'|Δvalve| (MA-{window_size})', line=dict(color='brown'))
            )
            fig_action.update_layout(title="Valve Action Magnitude", xaxis_title="Steps", yaxis_title="|Δvalve|", height=300)
...
def render_episode_analysis(episodes_df):
    ...
    if episodes_df is None or len(episodes_df) == 0:
        st.info("📊 Episode analysis will appear here after completing at least one episode.")
        st.markdown("""
        **What you'll see here:**
    ...
    # データテーブル
    with st.expander("📋 Episode Data Table", expanded=False):
    ...
    if 'mean_valve_change' in episodes_df.columns:
        fig_perf.add_trace(
            go.Scatter(x=episodes_df['episode'], y=episodes_df['mean_valve_change'],
                      mode='lines+markers', name='Mean |Δvalve|', line=dict(color='brown')),
            row=1, col=2
        )
    ...
    fig_perf.update_yaxes(title_text="Mean |Δvalve|", row=1, col=2)
...
def render_detailed_metrics(steps_df, episodes_df):
    ...
    if steps_df is not None and len(steps_df) > 0:
        ...
        st.info("📊 Statistical summary will appear here after collecting step data.")
    ...
    if episodes_df is not None and len(episodes_df) > 0:
        ...
        st.markdown("### Learning Progress Indicators")
        st.info("📊 Learning progress indicators will appear here after completing multiple episodes.")
    ...
    if steps_df is not None and len(steps_df) > 0 and 'exploration' in steps_df.columns:
        ...
        st.info(f"💡 {exploration_ratio:.1f}% of steps were exploratory actions")